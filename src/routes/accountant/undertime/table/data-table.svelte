<script lang="ts" generics="TData, TValue">
	...
	// functions
	async function saveUndertime() {
		const rows = table.getRowModel().rows.map((row) => row.original);
		console.log(rows);

		try {
			const res = await fetch('/API/POST/save-undertime', {
				method: 'POST',
				headers: {
			...
			if (!res.ok) {
				...
				toast.error(error.message || 'Failed to save undertime');
				return;
			}

			const data = await res.json();
			console.log(data);
			toast.success('Undertime saved successfully');
		} catch (e) {
			console.error(e);
			//@ts-ignore
			toast.error(e?.message || 'Failed to save undertime');
		}
	}
</script>
...
<div class="w-full space-y-2">
	<div class="flex flex-row gap-2">
		...
		<div class="flex items-center space-x-2">
			<Switch
				bind:checked={$editing_dtr}
				id="edit-mode"
				class=" data-[state=checked]:bg-emerald-500"
			/>
			<Label class="text-xs" for="edit-mode">Edit Mode</Label>
		</div>
	</div>
					...
					>
						{#each row.getVisibleCells() as cell (cell.id)}
							<Table.Cell
								...
								class="h-10 px-3 text-sm font-medium text-slate-700"
							>
					...
					<Table.Row class="hover:bg-transparent">
						<Table.Cell colspan={columns.length} class="text-center">
							<div class="flex flex-row items-center justify-end">
								<Button
									onclick={saveUndertime}
									disabled={!$editing_dtr}
									size="lg"
									class="rounded-lg bg-emerald-500 text-xs hover:bg-emerald-600"
