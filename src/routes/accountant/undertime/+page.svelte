<script lang="ts">
	...
	let employees = $state<EmployeeUndertime[]>([]);
	const tableData = $derived([...employees]);
	// functions
	$effect(() => {
		if (month_value && year_value) {
			fetch(`/API/GET/all-employees-undertime?month=${month_value}&year=${year_value}`)
				.then((res) => {
...
<div class="min-h-screen space-y-8 bg-slate-50 p-6 md:p-10">
	...
	<div class="flex flex-col gap-6 sm:flex-row sm:items-center sm:justify-between">
		<div class="space-y-1.5">
			<h1 class="text-4xl font-extrabold tracking-tight text-slate-900">Undertime</h1>
			<p class="text-sm font-medium text-slate-500">Edit full-time employee undertime</p>
		</div>
	...
	<!-- date selector -->
	<div>
		<p class="mb-3 text-sm font-medium">Filter undertime by month and year.</p>
