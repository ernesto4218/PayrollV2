<script lang="ts" generics="TData, TValue">
	...
	// functions
	async function saveDecutions() {
		const rows = table.getRowModel().rows.map((row) => row.original);
		console.log(rows);

		try {
			const res = await fetch('/API/POST/save-deductions', {
				method: 'POST',
				headers: {
			...
			console.error(e);
			//@ts-ignore
			toast.error(e?.message || 'Failed to save deductions');
		}
	}
</script>
...
<div class="w-full space-y-2">
	<div class="flex flex-row gap-2">
		...
		<div class="flex items-center space-x-2">
			<Switch
				bind:checked={$editing_deductions}
				id="edit-mode"
