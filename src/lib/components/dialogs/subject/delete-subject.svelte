...
<script lang="ts">
	...
	let { subjectId, subjectCode, subjectName, onDeleted }: Props = $props();
	...
	async function handleDelete() {
		loading = true;
		try {
			const res = await fetch(`/API/DELETE/subject/${subjectId}`, {
				method: 'DELETE'
			});
...
<Dialog.Root bind:open>
	...
	<Dialog.Content class="max-w-md overflow-hidden rounded-2xl border border-red-100 p-0">
		...
		>
			...
			<div>
				<Dialog.Title class="text-base font-semibold text-red-950">Delete subject</Dialog.Title>
		...
		<div class="px-6 py-5">
			<div class="flex gap-3 rounded-xl border border-amber-200 bg-amber-50 px-4 py-3.5">
				...
				<p class="text-[13px] text-amber-800">
					...
					<span class="font-semibold">{subjectName}</span>. This record and all associated data will
					be removed.
				</p>
			</div>
			...
			<div class="mt-4 rounded-xl border border-gray-200 bg-gray-50 px-4 py-3.5">
				<div class="flex flex-col gap-1.5">
					...
					<div class="flex items-center justify-between">
						<span class="text-[11px] font-semibold tracking-wide text-gray-400 uppercase">Name</span
						>
						<span class="max-w-[60%] truncate text-right text-[13px] text-gray-700"
							>{subjectName}</span
						>
					</div>
					<div class="flex items-center justify-between">
						<span class="text-[11px] font-semibold tracking-wide text-gray-400 uppercase">ID</span>
						<span class="font-mono text-[13px] text-gray-500">#{subjectId}</span>
					</div>
				</div>
			</div>
		</div>
