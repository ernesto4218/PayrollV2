...
<script lang="ts">
	...
	let { travelOrderId, employeeName, purpose, message, onDeleted }: Props = $props();
	...
	async function handleDelete() {
		loading = true;
		try {
			const res = await fetch(`/API/DELETE/travel-order/${travelOrderId}`, {
				method: 'DELETE'
			});
			...
			toast.success('C.A/Travel Order deleted successfully!');
			open = false;
			onDeleted?.();
		} catch (e: any) {
...
<Dialog.Root bind:open>
	...
	<Dialog.Content class="max-w-md overflow-hidden rounded-2xl border border-red-100 p-0">
		...
		>
			...
			<div>
				<Dialog.Title class="text-base font-semibold text-red-950"
					>Delete C.A/Travel Order</Dialog.Title
				>
		...
		<div class="px-6 py-5">
			<div class="flex gap-3 rounded-xl border border-amber-200 bg-amber-50 px-4 py-3.5">
				...
				<p class="text-[13px] text-amber-800">
					You are about to permanently delete the C.A/Travel Order for
					<span class="font-semibold">{employeeName}</span>. This record and all associated data
					will be removed.
				</p>
			</div>

			<!-- C.A/Travel Order details -->
				...
				<div class="flex flex-col gap-1.5">
					<div class="flex items-center justify-between">
						<span class="text-[11px] font-semibold tracking-wide text-gray-400 uppercase"
							>Employee</span
						>
						<span class="text-[13px] font-medium text-gray-700">{employeeName}</span>
					</div>
					<div class="flex items-center justify-between">
						<span class="text-[11px] font-semibold tracking-wide text-gray-400 uppercase"
							>Purpose</span
						>
						<span class="max-w-[60%] truncate text-right text-[13px] text-gray-700">{purpose}</span>
					</div>
					<div class="flex items-center justify-between">
						<span class="text-[11px] font-semibold tracking-wide text-gray-400 uppercase"
							>Message</span
						>
						<span class="max-w-[60%] truncate text-right text-[13px] text-gray-700">{message}</span>
					</div>
					<div class="flex items-center justify-between">
						<span class="text-[11px] font-semibold tracking-wide text-gray-400 uppercase">ID</span>
						<span class="font-mono text-[13px] text-gray-500">#{travelOrderId}</span>
					</div>
				</div>
			</div>
		</div>
